...
from app.domain.model import Batches, OrderLine, OutOfStock, allocate


def test_prefers_current_stock_batchs_to_shipment():
    in_stock_batch = Batches("in-stock-batch", "retro-clock", 100, eta=None)
    shipment_batch = Batches(
        "shipment-batch",
        "retro-clock",
        100,
        eta=datetime.datetime.today() + datetime.timedelta(days=1),
    )
...
def test_prefers_earlier_batchs():
    ...
    latest = Batches(
        ...
        "minimalist-spoon",
        100,
        eta=datetime.datetime.today() + datetime.timedelta(days=2),
    )
